...
#minFreq=""
        ...
        dataSource<-paste("resultsPowerOneBlocksimulationsDiffGeneralNewOnlyHetero_Size200", minFreq, "All", sep="")
...
range<-c(7,23, 15,16, 24) #  7: score TDT, 8: permutations TDT, 24: TDTEntropy, 15: LCT,  16: SRT

#yFirst=getYFirst(data, range, selectedRows, offset)
#yLast=getYLast(data, range, selectedRows, offset)
yFirst=0
yLast=1
makeOnePlot(1:length(recombinations), data, selectedRows, range, "Recombination fraction", "Association rate", yFirst, yLast, xLabels, title, offset, 0) 
...
name=paste(dir, "simulationsOldRisks", sep="")
